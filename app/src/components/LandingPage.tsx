...
const FEATURED_SCENARIOS = [
  { title: 'Review Board Defence', scenarioId: 'VALOUR-S01', description: 'Defend a design recommendation under scrutiny from delivery, risk, and executive stakeholders.', role: 'Technology Leader', duration: '12 min' },
  ...
  { title: 'Delivery Conflict', scenarioId: 'VALOUR-S06', description: 'Navigate disagreement, delivery pressure, and competing priorities without losing alignment.', role: 'Delivery Lead', duration: '10 min' },
];

const PATHWAYS = [
  { number: '01', title: 'Prepare', body: 'Frame the situation, decision, and stakeholder tension before entering the room.' },
  { number: '02', title: 'Engage', body: 'Respond to realistic challenge from an AI stakeholder pursuing a clear objective.' },
...
export function LandingPage({ onViewScenarios }: Props) {
  ...
  return (
    <div className="exec-site">
      <style>{`
        .exec-site { --paper:#f4f0e7; --paper-2:#ebe4d7; --ink:#151713; --graphite:#1b201c; --graphite-2:#242b25; --muted:#6d7068; --rule:rgba(21,23,19,.15); --gold:#b98a3b; --gold-soft:#d6b875; min-height:100vh; background:var(--paper); color:var(--ink); font-family:'DM Sans',system-ui,sans-serif; }
        .exec-site * { box-sizing:border-box; }
        .exec-shell { width:min(1440px,calc(100% - 48px)); margin:0 auto; }
        .exec-ecosystem { min-height:34px; display:flex; align-items:center; justify-content:space-between; padding:0 24px; background:#0f120f; color:rgba(255,255,255,.62); font-size:11px; letter-spacing:.12em; text-transform:uppercase; }
        .exec-ecosystem-links { display:flex; gap:22px; }
        .exec-ecosystem a { color:inherit; text-decoration:none; }
        .exec-nav { min-height:82px; display:grid; grid-template-columns:1fr auto 1fr; align-items:center; border-bottom:1px solid var(--rule); }
        ...
        .exec-btn { min-height:46px; padding:0 22px; border:1px solid transparent; border-radius:3px; font:600 13px 'DM Sans',sans-serif; cursor:pointer; box-shadow:0 8px 18px rgba(21,23,19,.1),inset 0 1px 0 rgba(255,255,255,.22); transition:transform .16s ease,background .16s ease,border-color .16s ease,box-shadow .16s ease; }
        ...
        .exec-hero { background:var(--graphite); color:#f7f2e8; min-height:690px; display:grid; grid-template-columns:minmax(0,1.12fr) minmax(380px,.88fr); align-items:stretch; }
        ...
        .exec-scenario { text-align:left; background:transparent; border:1px solid rgba(21,23,19,.18); border-radius:0; padding:30px; min-height:300px; display:flex; flex-direction:column; cursor:pointer; color:inherit; transition:background .16s ease,transform .16s ease; }
        ...
        @media (max-width:980px) { .exec-nav{grid-template-columns:1fr auto}.exec-nav-links{display:none}.exec-hero{grid-template-columns:1fr}.exec-room{min-height:520px;padding:54px 24px}.exec-hero-copy{padding:82px 24px 72px}.exec-intro-grid{grid-template-columns:1fr;gap:36px}.exec-path-grid,.exec-scenario-grid{grid-template-columns:1fr}.exec-path-card{border-right:0;border-bottom:1px solid var(--rule)}.exec-path-card:last-child{border-bottom:0}.exec-section-head{align-items:start;flex-direction:column} }
      ...
      <div className="exec-shell"><nav className="exec-nav" aria-label="Primary navigation"><div className="exec-brand">Valour&trade;<small>by Ordo Animi</small></div><div className="exec-nav-links"><button className="exec-nav-link" onClick={onViewScenarios}>Scenarios</button><button className="exec-nav-link" onClick={resumeLastSession}>Resume session</button></div><div className="exec-nav-actions"><button className="exec-btn exec-btn-ghost" onClick={resumeLastSession}>Resume</button><button className="exec-btn exec-btn-dark" onClick={onViewScenarios}>Choose scenario</button></div></nav></div>
      <section className="exec-hero"><div className="exec-hero-copy"><div className="exec-eyebrow">Agentic AI leadership sessions</div><h1>Enter the room <em>ready to lead.</em></h1><p>Valour places technology and senior leaders inside realistic, high-stakes conversations so they can test decisions, practise responses, and prepare a stronger next move.</p><div className="exec-hero-actions"><button className="exec-btn exec-btn-primary" onClick={onViewScenarios}>Choose a scenario</button><button className="exec-btn exec-btn-ghost" onClick={resumeLastSession}>Resume session</button></div><div className="exec-hero-proof"><span>Private practice</span><span>Responsive AI stakeholders</span><span>Structured debrief</span></div></div><div className="exec-room" aria-label="Product preview"><div className="exec-room-card"><div className="exec-room-kicker"><span>Live session</span><span>07:42 remaining</span></div><div className="exec-room-title">Review Board Chair</div><p className="exec-room-quote">&ldquo;You are asking us to approve significant delivery risk without proving the commercial return. Why should we support this?&rdquo;</p><div className="exec-objective"><div className="exec-objective-label">Your objective</div><p>Secure conditional endorsement while addressing cost, accountability, and delivery confidence.</p></div></div></div></section>
      <section className="exec-intro"><div className="exec-shell exec-intro-grid"><div><div className="exec-section-label">The leadership reality</div></div><div className="exec-intro-copy"><h2>Leadership is rarely difficult because information is absent.</h2><p>It is difficult because time is limited, interests conflict, and the quality of the decision depends on how it is communicated.</p><p>Valour creates a private place to practise what happens next&mdash;before the real conversation carries consequences.</p></div></div></section>
      <section className="exec-pathways"><div className="exec-shell"><div className="exec-section-head"><div><div className="exec-section-label">The Valour method</div><h2>Prepare. Engage. Debrief.</h2></div><p>A focused experience that moves from context to live pressure, then converts the session into a stronger real-world response.</p></div><div className="exec-path-grid">{PATHWAYS.map(path => <article className="exec-path-card" key={path.number}><div className="exec-path-number">{path.number}</div><h3>{path.title}</h3><p>{path.body}</p></article>)}</div></div></section>
      <section className="exec-scenarios"><div className="exec-shell"><div className="exec-section-head"><div><div className="exec-section-label">Executive case library</div><h2>Start with one real leadership moment.</h2></div><p>Review a situation that resembles the pressure you are preparing to face. No session begins until you confirm the scenario and preparation path.</p></div><div className="exec-scenario-grid">{FEATURED_SCENARIOS.map(scenario => <button className="exec-scenario" key={scenario.scenarioId} onClick={() => { window.location.hash = `#scenarios?scenario=${scenario.scenarioId}`; }}><div className="exec-scenario-meta"><span>{scenario.role}</span><span>{scenario.duration}</span></div><h3>{scenario.title}</h3><p>{scenario.description}</p><div className="exec-scenario-action">Review scenario &rarr;</div></button>)}</div><div className="exec-scenario-footer"><button className="exec-btn exec-btn-ghost" onClick={onViewScenarios}>View all scenarios</button></div></div></section>
      <section className="exec-final"><div className="exec-shell"><h2>The real conversation should not be your first attempt.</h2><p>Choose a leadership scenario, review the context, prepare your position, then enter the live session when you are ready.</p><button className="exec-btn exec-btn-primary" onClick={onViewScenarios}>Choose a scenario</button></div></section>
      <footer className="exec-shell exec-footer"><div className="exec-brand">Valour&trade;<small>by Ordo Animi</small></div><div>Leadership preparation &middot; The OrdoAnimi Group</div></footer>
    </div>
  );
}

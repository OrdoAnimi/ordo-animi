...
export function TopBar({ pilot, mode, custosOpen, onOpenCustos, onViewPattern, onViewReadiness, onViewComparison }: Props) {
  ...
  return (
    <header className="topbar">
      <button className="topbar-back" onClick={() => { window.location.hash = ''; }} title="Save and return home" aria-label="Save and return home">&larr;</button>
        ...
        <div className="topbar-meta">
          {pilot.evidence.startingConfidence != null && (
            <div className="topbar-metric">
              ...
              <span className="topbar-metric-value azure">{pilot.evidence.startingConfidence} &rarr; {pilot.evidence.endingConfidence}</span>
            </div>
          )}
          <div className="topbar-metric">
          ...
          <button className="btn btn-ghost topbar-pattern-btn" onClick={onViewComparison}>Compare &rarr;</button>
          <button className="btn btn-ghost topbar-pattern-btn" onClick={onViewReadiness}>Readiness</button>
          <button className="btn btn-ghost topbar-pattern-btn" onClick={onViewPattern}>Pattern &rarr;</button>
